...
export default function Dashboard() {
  ...
  async function handleFile(e) {
    ...
    if (tooBig.length > 0) {
      setCsvError(`❌ Fichier trop volumineux : ${tooBig.map(f => `${f.name} (${(f.size / (1024 * 1024)).toFixed(2)} MB)`).join(", ")} — maximum 10 MB.`);
      return;
    }
    setCsvError("");
    setData(null);
  ...
  : "Aucun fichier"}
                ...
                <div style={{ marginTop: "24px", display: "flex", gap: "12px", alignItems: "center" }}>
                  <button
                    onClick={runForecast}
                    disabled={loading || !!csvError || !data}
                    style={{ ...STYLE.btn("primary"), opacity: (loading || !!csvError || !data) ? 0.6 : 1 }}
                  >
